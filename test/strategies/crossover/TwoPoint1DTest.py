...
class TwoPoint1DTest(unittest.TestCase):
    def test_offsprings_absolute(self):
        s = crossover.TwoPoint1D(num_offsprings=40)
        pop = t.randn(100,400)
        popc = t.clone(pop)
        (newpop,), kargs = s(popc)
    ...
    def test_offsprings_fraction(self):
        s = crossover.TwoPoint1D(fraction_offsprings=0.4)
        pop = t.randn(100,400)
        popc = t.clone(pop)
        (newpop,), kargs = s(popc)
        self.assertEqual(newpop.shape, (100,400))
    ...
    def test_absolute_without_replace(self):
        s = crossover.TwoPoint1D(num_offsprings=40, replace_parents=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (140,400))

    def test_fraction_without_replace(self):
        s = crossover.TwoPoint1D(fraction_offsprings=0.4, replace_parents=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (140,400))
    ...
    def test_odd_offsprings(self):
        with self.assertRaises(ValueError):
            crossover.TwoPoint1D(num_offsprings=39)

    def test_offspring_count_not_set(self):
        with self.assertRaises(ValueError):
            crossover.TwoPoint1D()

    def test_offsprings_absolute_not_inplace(self):
        s = crossover.TwoPoint1D(num_offsprings=40, in_place=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (100,400))
    ...
    def test_fraction_absolute_not_inplace(self):
        s = crossover.TwoPoint1D(fraction_offsprings=0.4, in_place=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (100,400))
        self.assertIsNot(pop, newpop)
